  ...
  $.ajax({
    ...
    success: function (chatRooms) {
      ...
      chatRooms.forEach(function (room) {
        const formattedDate = room.recruitmentDeadline ? new Date(room.recruitmentDeadline).toLocaleString() : '없음';
        ...
        const html = `
                    <div className="chat-item" 
                        ...
                        <div className="chat-info">
                            <span className="chat-location">${room.location || '없음'}</span>
                            <span className="chat-name">${room.roomName || '없음'}</span>
                        </div>
      ...
      $('.chat-item')
        ...
        .on('click', function (event) {
          ...
          if (detailsContainer.length === 0) {
            detailsContainer = $(`
                        <div className="chat-details-container" data-room-id="${roomId}">
                            <p>생성자: ${creator}</p>
                            <p>마감일: ${deadline}</p>
							<p>테마: ${theme}</p>
                        </div>
