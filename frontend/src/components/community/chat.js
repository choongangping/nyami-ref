...
function loadChatRooms() {
  ...
  $.ajax({
    ...
    success: function (rooms) {
      ...
      rooms.forEach((room) => {
        const roomHtml = `
			        <div className="chat-room" data-room-id="${room.chatRoomId}">
			            <div className="chat-room-header">
			                <div className="room-name">${room.roomName} (${room.currentParticipants}/${room.maxParticipants})</div>
			                <div className="last-time">${formatTimestamp(room.lastMessageSentAt) || '없음'}</div>
			            </div>
			            <div className="last-message">${room.lastMessage || '마지막 메시지가 없습니다.'}</div>
			        </div>
			    `;
...
function displayMessage(message) {
  ...
  const messageHtml = `
	    <div id="message-${message.timestamp}" className="chat-message ${messageClass}">
	        <div className="chat-sender">${message.sender}</div>
	        <div className="chat-content">${message.content}</div>
	        <div className="chat-timestamp">${formattedTimestamp}</div>
	    </div>
	`;
