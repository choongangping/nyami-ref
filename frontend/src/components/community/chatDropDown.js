$(document).ready(function () {
  ...
  window.filterByIndustry = function (value, displayName) {
    ...
    if (subCategoryOptions[value]) {
      subCategoryOptions[value].forEach((option) => {
        subCategoryMenu.append(`<a href="#" onClick="filterBySubCategory('${option}', '${option}')">${option}</a>`);
      });
    }
  ...
  function applyFilters() {
    ...
    $('.chat-item').each(function () {
      ...
      const matchLocation = !filters.location || filters.location === '전체' || chatLocation === filters.location;
      const matchIndustry = !filters.industry || chatThemes.includes(filters.industry);
      const matchSubCategory = !filters.subCategory || chatThemes.includes(filters.subCategory);
      const matchThemes = !filters.theme.length || filters.theme.some((theme) => chatThemes.includes(theme));
