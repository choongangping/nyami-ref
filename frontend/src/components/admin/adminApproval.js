$(() => {
  ...
  function renderStorePopup(store) {
    const themeHtml = store.themes && store.themes.length > 0 ? store.themes.join(', ') : '테마 정보가 없습니다.';

    const mainImageHtml = store.mainImage1
      ? `<img src="/images/store/${store.mainImage1}" alt="가게 대표 사진">`
      : `<div id="storeImages" className="map-container" style="margin-bottom: 50px;">입력된 가게 사진이 없습니다.</div>`;

    const menuImageHtml = store.menuImage
      ? `<img src="/images/store/${store.menuImage}" alt="대표 메뉴 사진">`
      : `<div id="storeImages" className="map-container">입력된 대표 메뉴 사진이 없습니다.</div>`;

    const btnHtml =
      store.enrollStatus == 'wait' || store.enrollStatus == 'read'
        ? `<div style="display: flex; gap: 80px;">
         <button className="btn btn-submit" data-id="${store.id}">등록</button>
         <button className="btn btn-reject" data-id="${store.id}">반려</button>
       </div>`
        : `<div style="display: flex; justify-content: center; align-items: baseline; gap: 20px;">
         ...
         <button className="btn" style="background-color: #79f; width: 60px;" onClick="closePopup()">닫기</button>
       </div>`;
