...
function selectIndustry(industry) {
  ...
  const selectedButton = Array.from(document.querySelectorAll('.category-step button')).find((btn) => btn.textContent.includes(industry));
  if (selectedButton) {
...
function selectSubCategory(subCategory) {
  ...
  const selectedButton = Array.from(document.querySelectorAll('.subcategory-btn')).find((btn) => btn.textContent.trim() === subCategory);
  if (selectedButton) {
...
function selectTheme(theme) {
  ...
  if (themeIndex === -1) {
    ...
    const selectedButton = Array.from(document.querySelectorAll('#themeStep button')).find((btn) => btn.textContent.includes(theme));
    if (selectedButton) {
      selectedButton.classList.add('active');
    }
  } else {
  ...
  $.ajax({
    ...
    success: function (stores) {
      ...
      stores.forEach(function (store) {
        html +=
          '<div className="store-item-box" onClick="goToStoreDetail(' +
          store.id +
          ')">' +
...
//                let html = '';
//                stores.forEach(function(store) {
//                    html += '<div className="store-item-box" onClick="goToStoreDetail(' + store.id + ')">' +
//                                '<div className="store-item">' +
