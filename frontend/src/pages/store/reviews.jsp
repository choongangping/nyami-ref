...
<html>
  <head>
    ...
    <script>
        ...
        $.each(reviews, function (index, review) {
          if (review.status === 'active') {
            ...
            if (review.reviewImage) {
              ...
              reviewItem += '<div className="review-images">';
            ...
            // 수정, 삭제 버튼 추가
            if (review.memberId != null && review.memberId === memberId) {
              // 본인이 작성한 리뷰일 경우에만 삭제 버튼 표시
              console.log('review.memberId ', review.memberId);
              console.log('review id: ', review.id);
              reviewItem += '<button className="edit-review-button" onClick="editReview(' + review.id + ", '" + review.content + '\')">수정</button>';
              reviewItem += '<button className="delete-review-button" onClick="hiddenReview(' + review.id + ', ' + review.memberId + ')">삭제</button>';
            }
      ...
      function generateStars(rating) {
        var stars = '';
        for (var i = 1; i <= 5; i++) {
          stars += i <= rating ? '<span className="star filled">★</span>' : '<span className="star">★</span>';
        }
        return stars;
      }
  ...
  <body>
    <div className="section review-section">
      <div className="review-title">리뷰 목록 (<span id="reviewCount">0</span>)</div>
