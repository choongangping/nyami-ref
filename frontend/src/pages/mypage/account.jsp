		...
		<div className="content">
			...
			<div className="main-content">
				...
				<div className="expanded-content">
					<!-- 계정 정보 섹션 -->
					<div id="account-settings" className="section">
						<div className="div-content">
							<h3>이메일 정보</h3>
								<input type="email" id="userEmail" name="userEmail" value="${sessionMember.email}">
								<!-- <button className="email-verify-button" onClick="sendVerificationEmail()">이메일 인증</button> -->
								<button type="button" className="email-verify-button">이메일 인증</button>
								...
								<c:choose>
									<c:when test="${empty sessionMember.naverId}">
										<a href="/oauth2/authorization/naver" onClick="setRedirectUrl('/updateSocialId','네이버')">
											<img src="/images/naver_button.png" alt="네이버 간편 로그인" className="login-btn">
											네이버 연결하기
										</a>
									</c:when>
									<c:otherwise>
										<a style="cursor: default;">
											<img src="/images/naver_button.png" alt="네이버 간편 로그인" className="enrolled-login-btn">
											네이버 등록완료
										</a>
									</c:otherwise>
								</c:choose>
								<c:choose>
									<c:when test="${empty sessionMember.kakaoId}"> 
										<a href="/oauth2/authorization/kakao" onClick="setRedirectUrl('/updateSocialId','카카오')">
											<img src="/images/카카오.png" alt="카카오 간편 로그인" className="login-btn">
											카카오 연결하기
										</a>
									</c:when>
									<c:otherwise>
										<a style="cursor: default;">
											<img src="/images/kakao_button.png" alt="카카오 간편 로그인" className="enrolled-login-btn">
											카카오 등록완료
										</a>
									</c:otherwise>
								</c:choose>
								<c:choose>
									<c:when test="${empty sessionMember.googleId}">
										<a href="/oauth2/authorization/google" onClick="setRedirectUrl('/updateSocialId','구글')">
											<img src="/images/구글.png" alt="구글 간편 로그인" className="login-btn">
											구글 연결하기
										</a>
									</c:when>
									<c:otherwise>
										<a style="cursor: default;">
											<img src="/images/google_button.png" alt="구글 간편 로그인" className="enrolled-login-btn">
										구글 등록완료
										</a>
									</c:otherwise>
								</c:choose>
						...
						<div className="div-content">
							...
							<form action="/account" method="post">
								...
								<input type="password" id="current_password" name="current_password" className="${sessionMember.passwd == null ? 'disabled' : ''}" >
						...
						<div className="div-content">
							<h3>계정삭제</h3>
							<textarea className="policy-text" readonly>"회원 탈퇴일로부터 계정과 닉네임을 포함한 계정 정보(아이디/이메일/닉네임)는 개인정보 처리방침에 따라 60일간 보관(잠김)되며, 
