...
<div className="sidebar">
  <div className="profile-pic" onClick="document.getElementById('fileInput').click()">
    <c:if test="${not empty sessionMember.profileImage}">
      <img src="${sessionMember.profileImage}" alt="프로필 사진" /> <span className="profile-overlay">프로필 변경</span>
      <input type="file" id="fileInput" style="display: none" onChange="uploadFile(event)" />
    </c:if>
    <c:if test="${empty sessionMember.profileImage}">
      <img src="/images/profile.png" alt="프로필 사진" /> <span className="profile-overlay">프로필 변경</span>
      <input type="file" id="fileInput" style="display: none" onChange="uploadFile(event)" />
    </c:if>
  </div>
  <div
    className="memberChange"
    id="memberChange"
    style="background-color: #bfbfbf; color: #fff; border-radius: 7px; width: 110px; margin: 0 auto; margin-top: 20px"
    onClick="memberCategoryChange()"
  >
    ${sessionMember.category}회원
  </div>
  <div className="profile-name">${sessionMember.nickname}</div>
  <div className="profile-point">내 포인트 : ${point}</div>
  <div className="profile-intro">${sessionMember.introduction}</div>
</div>
